...
function getPolygonsBySlotId(cpSlotId) {
  ...
  return polygons.map((row) => {
    // FIX: Convert DB format {lat, lng} directly to Turf/GeoJSON standard [lng, lat] for geometry consistency
...
function checkCarInsideSpecificSlot(currentLat, currentLng, slotPolygon, slotNumber, facilityId) {
  ...
  try {
    // FIX: slotPolygon.coords is already [lng, lat] from getPolygonsBySlotId, NO RE-MAPPING NEEDED
...
function checkCarInsideFacility(currentLat, currentLng, facilityPolygons, facilityId) {
  ...
  for (let polyObj of facilityPolygons) {
    try {
      // FIX: polyObj.coords is already [lng, lat] from getPolygonsByFacilityId, NO RE-MAPPING NEEDED
...
async function handleFacilityStateUpdate(carRow, lat, lon, isMoving, eventName = 'Location Update') {
  ...
  let checkMode = `Facility (All Slots) from getPolygonsByFacilityId where get the facilityIdCheck',${facilityIdCheck,validPolygons}`; 
