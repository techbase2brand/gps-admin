...
const PARK_CHECK_DELAY_MS = 3600000; 
 ...
 * and transforms them into an array of coordinate objects including the slot number.
 * @returns {Array<{slot_number: number, coords: Array<[number, number]>, id: number}>} Array of processed polygon objects.
 */
function getFacilityPolygonsForCar(cpSlot) { // Function name is misleading, it now fetches by cpSlot ID (id column)
  if (!cpSlot) return [];
  
  const cpSlotString = String(cpSlot); // cpSlot (from DB cars.cpSlot) contains the facility_polygons.id
  
  // NOTE: This logic assumes that cars.cpSlot stores the ID (bigint) of the facility_polygons table row.
  const carFacilityPolygons = facilityPolygonsCache.filter(
    (e) => String(e.id) === cpSlotString
  );
  
  // LOG FIX: Removed incorrect 'facilityId' reference
  // console.log("getFacilityPolygonsForCar +++++", cpSlot, carFacilityPolygons); 
 ...
 * Returns the matching slot number if found.
 */
function checkCarInsideFacility(currentLat, currentLng, facilityPolygons, facilityId) {
  ...
  if (isNaN(currentLat) || isNaN(currentLng)) {
    console.error("Invalid car coordinates:", currentLat, currentLng);
    return { inside: false, facilityIdCheck: null, slotNumber: null };
  }
  ...
  for (let polyObj of facilityPolygons) {
    try {
      ...
      if (booleanPointInPolygon(carPoint, turfPolygon)) {
        ...
        console.log(`   Location: [Lng: ${currentLng}, Lat: ${currentLat}]`);
        console.log(`   ✅ INSIDE Facility ${facilityId}, Slot ${polyObj.slot_number}`);
        return { 
            inside: true, 
  ...
  if (facilityPolygons.length > 0) {
    ...
    const polygonCount = facilityPolygons.length;
    console.log(`   ❌ OUTSIDE all ${polygonCount} polygons for Facility ${facilityIdString}`);
  }
  return { inside: false, facilityIdCheck: null, slotNumber: null };
}
...
async function sendNotification(chipId, facilityName, carData, slotNumber) {
  try {
    const message = `Car with VIN ${carData?.vin} and chip id ${carData?.chip} has left ${facilityName}, Slot ${slotNumber}.`;
...
async function checkParkedStability(chipId) {
    ...
    if (isStable && is_inside_facility) {
        ...
        const slotNumber = cpSlot; // cpSlot now holds the integer slot number
...
async function checkSlotExitNotification(carRow, lat, lon) {
    ...
    console.log(`\n--- [${chipId}] Checking Slot Exit for Slot ${cpSlot} ---`);

    // Get facility polygons
    const validPolygons = getFacilityPolygonsForCar(cpSlot);
    if (validPolygons.length === 0) {
        console.warn(`[${chipId}] No facility polygons found for cpSlot ${cpSlot}. Cannot check slot exit.`);
        return false;
    }

    // Find the specific slot polygon (Note: getFacilityPolygonsForCar should only return one polygon now)
    const slotPolygon = validPolygons[0]; 
    
    // Log the specific slot coordinates for debugging (User Request)
    console.log(`   Slot ${cpSlot} Polygon Coords: ${JSON.stringify(slotPolygon.coords)}`);


    // Check if car is currently outside this specific slot
    const slotCheck = checkCarInsideSpecificSlot(lat, lon, slotPolygon, cpSlot, facilityId);

    // If car is outside the slot, send notification
    if (!slotCheck.inside) {
        console.log(`\n 🚨 ALERT: Car ${chipId} has LEFT Slot ${cpSlot}!`);
        
        const facilityName = await getFacilityName(facilityId);
        
        // Send notification (this will set notification_sent_check to true in DB)
        const notificationSentSuccessfully = await sendNotification(chipId, facilityName, carRow, cpSlot);
        ...
        console.log(`[${chipId}] Car is still inside Slot ${cpSlot}. No notification needed.`);
...
async function handleFacilityStateUpdate(carRow, lat, lon, isMoving, eventName = 'Location Update') {
  ...
  console.log(`   DB State: cpSlot=${carRow.cpSlot}, is_moving=${carRow.is_moving}, PrevInside=${carRow.previousIsInsideFacility}`);
  ...
  // This updates the database asynchronously if an exit occurs.
  // ====================================================================
  let notificationTriggered = false;
  
  // Slot Exit Check should only run if car is moving (is_moving = true) and triggered by GPS Update (419x)
  if (carRow.cpSlot !== null && carRow.is_moving === true && eventName === 'GPS Update') {
     ...
     if (exited) {
        ...
        // NOTE: This refetch ensures that the subsequent DB update (Line 493) uses the latest, cleared cpSlot=null state.
  ...
  const checkResult = validPolygons.length > 0
    ...
    : { inside: false, facilityIdCheck: null, slotNumber: null }; // Default to outside if no polygons loaded

  const currentInside = checkResult.inside ?? false;
  const detectedFacilityId = currentInside ? facilityIdCheck : null;
  // Detected Slot is only relevant for END MOVEMENT to mark the final parked spot.
  const detectedSlot = checkResult.slotNumber; 
  ...
  const updateObj = {
    ...
    // Use the *newly detected slot* ONLY if currently inside. 
    // Otherwise, carry forward the updated cpSlot (which might be null if cleared by checkSlotExitNotification).
    cpSlot: currentInside ? detectedSlot : carRow.cpSlot || null, 
  ...
  if (isEndMovement) {
    ...
    updateObj.cpSlot = detectedSlot; // Use the newly detected slot for parking

    console.log(`[${chipId}] END MOVEMENT -> is_moving = FALSE. Flags reset. Parked location saved. Parked Lng: ${lon}, Parked Lat: ${lat}.`);

    // Schedule the stability check for 1 hour later (if car is inside a slot)
    if (detectedSlot && currentInside) {
  ...
  if (updateError) {
    ...
    console.log(`UPDATED chip ${chipId} | Moving: ${updateObj.is_moving} | PrevInside: ${previousInside} | CurrentInside: ${currentInside} | Slot: ${finalCpSlot} | NotifTriggered: ${notificationTriggered} | ParkCheckScheduled: ${isEndMovement && currentInside ? 'YES' : 'NO'}`);
  }
}
...
async function startMqttWorker() {
  ...
  console.log("++facilityPolygonsCache",facilityPolygonsCache);
  ...
  client.on("message", async (topic, message) => {
    try {
      ...
      // ============================================================
        ...
        // This cached state is only for latency reduction; the database is the source of truth.
