...
function checkCarInsideFacility(currentLat, currentLng, facilityPolygons, facilityId) {
  // ... (unchanged code for coordinates and carPoint) ...
  currentLat = parseFloat(currentLat);
  currentLng = parseFloat(currentLng);
...
async function handleFacilityStateUpdate(carRow, lat, lon, isMoving, eventName = 'Location Update') {
  ...
  // FIX: Use the correct function to filter by facility ID
  ...
  if (updateError) {
    ...
    console.log(`UPDATED chip ${chipId} | Moving: ${updateObj.is_moving} | PrevInside: ${previousInside} | CurrentInside: ${currentInside} | Slot ID: ${finalCpSlot} | NotifTriggered: ${notificationTriggered} | ParkCheckScheduled: ${isEndMovement && currentInside ? 'YES' : 'NO'}`);
  }
}
