...
const PARK_CHECK_DELAY_MS = 5000; 
...
function getPolygonsBySlotId(cpSlotId) {
  ...
  return polygons.map((row) => {
    // Convert DB format {lat, lng} directly to Turf/GeoJSON standard [lng, lat] for geometry consistency
    const coords = row.coordinates.map((c) => [c.lng, c.lat]);
    return {
 ...
 */
function getPolygonsByFacilityId(facilityId) { 
...
function checkCarInsideSpecificSlot(currentLat, currentLng, slotPolygon, slotNumber, facilityId) {
  ...
  try {
    const fixedCoords = slotPolygon.coords; 
...
function checkCarInsideFacility(currentLat, currentLng, facilityPolygons, facilityId) {
  ...
  for (let polyObj of facilityPolygons) {
    try {
      const fixedCoords = polyObj.coords;
      const closedPolygon = closePolygon(fixedCoords);
      const turfPolygon = polygon([closedPolygon]);
...
async function sendNotification(chipId, facilityName, carData, slotNumber) {
  ...
  try {
    ...
    if (tokensToSend.length > 0) {
      ...
      // This flag confirms the exit alert was SENT.
      ...
      console.log('DB updated: notification_sent_check set to TRUE.');
...
async function sendParkedNotification(chipId, facilityName, carData, slotNumber) {
    try {
        ...
        const message = `Car with VIN ${carData?.vin} and ${carData?.chip} is confirmed parked in ${facilityName}, Slot ${slotNumber}.`;
        console.log("message",message);
...
async function checkSlotExitNotification(carRow, lat, lon) {
    ...
    if (cpSlot === null || cpSlot === undefined) {
        ...
        return false; // No slot assigned, skip
    }

    if (carRow.notification_sent_check === true) {
         console.log(`[${chipId}] Slot Exit Check skipped: Notification already sent (notification_sent_check=true).`);
         return false;
    }
    
    // CRITICAL FIX: Ensure a Parked notification was sent before alerting for an exit.
    if (carRow.parkedNotification !== true) {
        console.log(`[${chipId}] Slot Exit Check skipped: Car was in slot ${cpSlot} but no 'Parked' alert was confirmed.`);
        return false;
    }
    ...
    // Log the specific slot coordinates for debugging
    ...
    if (!slotCheck.inside) {
        ...
        if (notificationSentSuccessfully) {
             // Clear cpSlot and parkedNotification in database since car left the slot.
            await supabase
                .from("cars")
                .update({
                    cpSlot: null,
                    parkedNotification: false, // <-- CRITICAL FIX: Reset parked notification flag
                    // notification_sent_check is set in sendNotification
                })
                .eq("id", carRow.id); // Use carRow.id to update the specific row
            
            console.log(`[${chipId}] Slot exit notification sent and state cleared.`);
            ...
            console.log(`[${chipId}] Notification failed. Skipping state clear.`);
            return false;
        }
    } else {
...
async function handleFacilityStateUpdate(carRow, lat, lon, isMoving, eventName = 'Location Update') {
  ...
  // CRITICAL FIX: Use the calculated 'isMoving' state (which is true for a Start event) 
  // instead of the stale 'carRow.is_moving' which is false when parked.
  if (carRow.cpSlot !== null && isMoving === true && isMovementRelatedEvent) { 
     ...
     if (exited) {
        ...
        const { data: updatedCarRow } = await supabase
          .from("cars")
          .select("notification_sent_check, cpSlot, parkedNotification")
          .eq("chip", chipId)
          .single();
        ...
        if (updatedCarRow) {
            ...
            carRow.parkedNotification = updatedCarRow.parkedNotification; // Should be false
        }
     }
  }
  ...
  // MODIFIED to only fire if the car was NOT in a specific slot (cpSlot is null).
  if (!isEndMovement && previousInside && !currentInside && carRow.cpSlot === null && !notificationTriggered) {
    ...
    await sendNotification(chipId, facilityName, carRow, carRow?.cpSlot);
    notificationTriggered = true;
  }
