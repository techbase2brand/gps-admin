...
//   try {
//     const message = `Car with VIN ${carData?.vin} and chip ${chipId} has left ${facilityName}`;
    
//     // Using the DUMMY_FCM_TOKEN defined globally as per user request
    
//      const tokensToSend = await supabase.from("user_fcm_tokens").select("user_id","fcm_token","is_active").eq("is_active",true);
//     console.log("++++",tokensToSend);
    
//     if (tokensToSend.length > 0) {
      
//       // Send notification to the list of active tokens (FCM supports multicast)
...
async function sendNotification(chipId, facilityName, carData) {
  try {
    const message = `Car with VIN ${carData?.vin}  and chip id ${carData?.chipId}has left ${facilityName} slot nmber`;
    
    // FETCH ACTIVE TOKENS (User Request)
    const { data: tokensData, error: fetchError } = await supabase
        .from("user_fcm_tokens")
        .select("fcm_token");

    if (fetchError) {
        console.error("ERROR fetching active FCM tokens for notification:", fetchError.message);
        return; 
    }

    const tokensToSend = tokensData.map(row => row.fcm_token).filter(token => token);
    console.log(`++++ Fetched ${tokensToSend.length} active tokens for broadcast.`);
    ...
    if (tokensToSend.length > 0) {
      ...
      console.log(`FCM Broadcast Sent successfully to ${response.successCount} tokens.`);
    } else {
 ...
 * @param {boolean} isMovingFromEvent - Boolean state calculated from the MQTT event (5003). For 419x, this comes from the local cache.
 */
async function processLocationCheck(chipId, eventName, isMovingFromEvent) {
    ...
    if (eventName === 'GPS Update') {
        // FIX: For simple GPS updates, the source of truth for 'is_moving' must be the database.
        // This respects manual changes or the last processed 5003 event.
